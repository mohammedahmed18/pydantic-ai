...
def _estimate_string_tokens(content: str | Sequence[UserContent]) -> int:
    ...
    if isinstance(content, str):
        return len(TOKEN_SPLIT_RE.split(content.strip()))
    tokens = 0
    for part in content:
        if isinstance(part, str):
            tokens += len(TOKEN_SPLIT_RE.split(part.strip()))
        elif isinstance(part, BinaryContent):
            tokens += len(part.data)
        # Token count for AudioUrl, ImageUrl, and unhandled types stays zero (no-op).
    return tokens
